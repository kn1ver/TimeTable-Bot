...
async def post_timetable(file_name: str, callback: CallbackQuery, bot: Bot):
    ...
    topic = callback.message.message_thread_id
...
async def daemon(callback: CallbackQuery, bot: Bot, logger=logger_default):
    chat_id = callback.message.chat.id
    long_sleep = False
    api_error = False
    ...
    if not last_anns: last_anns = {str(chat_id): {'id': '-1'}}
    try: last_anns[str(chat_id)]
    except KeyError: last_anns[str(chat_id)] = " "
    ...
    if dt_1[0] < now < dt_1[1] or dt_2[0] < now < dt_2[1]:
        ...
        try: last_ann = await get_announcements()
        except Exception as e:
            logger.error(f"API Ошибка: {e}\n\n", exc_info=True)
            api_error = True
        logger.debug(last_ann)

        if  launch_type != "autostart" and not api_error and last_ann[list(last_ann)[0]]['id'] != last_anns[str(chat_id)]:
            last_anns[str(chat_id)] = last_ann[list(last_ann)[0]]['id']
            with open("data.json", "w") as f:
                data = {"launch_type": launch_type, "last_anns": last_anns}
            ...
            await post_timetable(file_name=json_data['name'], callback=callback, bot=bot)

            os.remove(f"files/{json_data['name'][:-5]}.png")
            os.remove(f"files/{json_data['name']}")
            long_sleep = True
        else:
            api_error = False
            long_sleep = False
            with open("data.json", "w") as f:
                data = {"launch_type": "handler", "last_anns": last_anns}
                json.dump(data, f, ensure_ascii=False, indent=4)
